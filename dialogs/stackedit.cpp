...
{
    connect(ui->coboxStackOrder, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index){
        if (index != -1)
        {
            saveStackInfo();
        }
    });
    connect(ui->coboxStopAfterStack, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index){
        if (index != -1)
        {
            saveStackInfo();
        }
    });
    connect(ui->coboxLeaveBlank, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index){
        if (index != -1)
        {
            saveStackInfo();
        }
    ...
    for (int i = 0; i < STACK_AXIS_NUM; i++)
    {
        connect(axisSelect[i], QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index){
            if (index != -1)
            {
                saveStackInfo();
            }
        });
        connect(stackDirect[i], QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index){
            if (index != -1)
            {
                saveStackInfo();
            }
        ...
        connect(speed[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(pointNum[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(speedDischage[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(startPosA[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(endPosB_X[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(endPosC_Y[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(posD[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(startPosDischage[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(intervalDis[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
        connect(offset[i], &NumberEdit::editingFinished, this, &StackEdit::saveStackInfo);
    }
}
