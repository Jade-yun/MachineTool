...
void Usart::ExtendSendParDeal(uint8_t mainCmd, uint8_t sunCmd, uint16_t parNum, uint16_t parNum2)
{
    ...
    {//安全设置
        ...
        case CMD_SUN_SAVE_ONLINE://联机安全
            ...
            num = parNum - 1;
            len = 1;
            sendDataBuf[1] = m_OnlineSave[num].mainSunMode;
            len += 1;
            sendDataBuf[len] = (uint8_t)m_OnlineSave[num].pluseTime;
            sendDataBuf[len+1] = (uint8_t)(m_OnlineSave[num].pluseTime>>8);
            len += 2;
            sendDataBuf[1] = m_OnlineSave[num].onlineSelect;
            len += 1;
            sendDataBuf[1] = m_OnlineSave[num].axisNum;
            len += 1;
            sendDataBuf[1] = m_OnlineSave[num].areaInNum;
            len += 1;
            sendDataBuf[1] = m_OnlineSave[num].areaOutNum;
            len += 1;
            sendDataBuf[1] = m_OnlineSave[num].requestInNum;
            len += 1;
            sendDataBuf[1] = m_OnlineSave[num].requestOutNum;
            len += 1;
    ...
    {//产品设置
        ...
        case CMD_SUN_PRODUCT_PAR://产品参数
            sendDataBuf[0] = m_ProductSet.waitProcessFinTime;
            sendDataBuf[1] = m_ProductSet.pluseOutTime;
            sendDataBuf[2] = m_ProductSet.airVlaveLimitTime;
            sendDataBuf[3] = m_ProductSet.autoCycTime;
            sendDataBuf[4] = m_ProductSet.alarmBuzzerTime;
            sendDataBuf[5] = m_ProductSet.chainReachDelay;
            sendDataBuf[6] = m_ProductSet.clampCheckFilterTime;
            len = 7;
            break;
        case CMD_SUN_PRODUCT_SENIOR://高级
            sendDataBuf[0] = m_ProductSenior.lubricOpenTime;
            sendDataBuf[1] = m_ProductSenior.lubricCloseTime;
            sendDataBuf[2] = m_ProductSenior.lubricDelay;
            sendDataBuf[3] = m_ProductSenior.lubricGapCyc;
            sendDataBuf[4] = m_ProductSenior.productMemoryFlag;
            break;
    ...
    {//伺服速度
        ...
        case CMD_SUN_SERVO_ACC_DEC://加减速时间
            ...
            num = parNum - 1;
            len = 1;
            if(parNum < AXIS_TOTAL_NUM)
            {//轴
                sendDataBuf[len] = (uint8_t)m_AxisPar[num].accTime;
                sendDataBuf[len+1] = (uint8_t)(m_AxisPar[num].accTime>>8);
                sendDataBuf[len+2] = (uint8_t)(m_AxisPar[num].accTime>>16);
                sendDataBuf[len+3] = (uint8_t)(m_AxisPar[num].accTime>>24);
                len += 4;
                sendDataBuf[len] = (uint8_t)m_AxisPar[num].decTime;
                sendDataBuf[len+1] = (uint8_t)(m_AxisPar[num].decTime>>8);
                sendDataBuf[len+2] = (uint8_t)(m_AxisPar[num].decTime>>16);
                sendDataBuf[len+3] = (uint8_t)(m_AxisPar[num].decTime>>24);
                len += 4;
    ...
    {//机器参数
        ...
        case CMD_SUN_MAC_ORIGIN://原点
            ...
            len += AXIS_TOTAL_NUM;
            sendDataBuf[index] = m_ServoPar.resetSunProFlag;
            len += 1;
            sendDataBuf[index] = m_ServoPar.torqueLimitFlag;
            len += 1;
            break;
        default:
            break;
...
//解析——参数
void ExtendReadParDeal(uint8_t mainCmd, uint8_t sunCmd, uint8_t *recDataBuf)
{
    uint16_t i = 0;
    uint16_t num = 0;
    uint8_t axis = 0;
...
void Usart::ExtendSendProDeal(uint8_t mainCmd, uint8_t sunCmd, uint16_t parNum, uint16_t parNum2, uint16_t parNum3)
{
    ...
    if(mainCmd == CMD_MAIN_PRO)
    {
        ...
        case CMD_SUN_PRO_INSERT://命令行插入、修改
            GetProData(parNum, parNum2, sendDataBuf, len,PRO_FLAG_CMDLINE_04,(uint8_t)parNum3);
            break;
