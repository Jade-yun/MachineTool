...
P_LogicIfStruct Temp_LogicIfStruct = {0};                                                   //教导界面高级-如果指令
P_LogicWhileStartStruct Temp_LogicWhileStartStruct = {0};                                   //教导界面高级-循环指令
P_LogicVarStruct Temp_LogicVarStruct = {0};                                                 //教导界面高级-逻辑&变量-变量-变量命令
P_LogicAxisStruct Temp_LogicAxisStruct = {0};                                                 //教导界面高级-逻辑&变量-变量-轴命令
P_LogicStackStruct Temp_LogicStackStruct = {0};                                                 //教导界面高级-逻辑&变量-变量-堆叠命令
P_LogicCurProductNumStruct Temp_LogicCurProductNumStruct = {0};                           //教导界面高级-逻辑&变量-变量-实际产量命令
P_LogicTimeStruct Temp_LogicTimeStruct = {0};                                                  //教导界面高级-逻辑&变量-变量-计数器命令
bool OrderNeedSaveFlag = false;//指令修改过提示是否需要保存
...
P_ProOrderStruct m_ProOrder[PRO_NUM][PRO_LINE_MAIN] = {0};				//主程序命令
P_GuideStruct m_Guide[GUIDE_TOTAL_NUM] = {0};                           // 操作指引
...
uint8_t  m_BackOriginOrderCnt = 0;																			//回零顺序计数器
uint8_t  m_BackOriginStep[AXIS_TOTAL_NUM] = {0};												//回零流程步骤号
uint8_t  m_BackOriginFinishFlag[AXIS_TOTAL_NUM] = {0};									//回零完成标志
uint8_t  m_BackOriginLastFlag[AXIS_TOTAL_NUM] = {0};										//回零最后一步标志

uint8_t  m_ManualMoveCmdFlag = 0;																			//手动模式标志，0不选择 1连动开始 2连动结束 3手轮 4增量 5移至堆叠点 100运动中 101减速中
uint8_t  m_ManualAxisNum = 0;																					//手动模式选中的轴编号，0-5
uint8_t  m_ManualMoveDir = 0;																					//手动运动-方向，0负向 1正向
uint16_t m_ManualMoveSpeed = 0;																				//手动运动-速度，0.1-100.0，单位0.1%
int32_t m_ManualMoveTarPos = 0;																				//手动运动-目标位置
uint8_t  m_ManualAllowDown_Z = 0;																			//手动运动-无机床'加工完成'信号允许轴下降，0不允许 1允许
uint8_t  m_ManualMultiplySpeed = 0;																		//手动运动-手轮倍率
uint32_t m_ManualIncrementDis = 0;																			//手动运动-增量距离
...
bool CircularBuffer_IsFull() {
  return (m_cb.head + 1) % BUFFER_SIZE == m_cb.tail;
}
// 判断环形缓冲区是否为空
...
// 获取环形缓冲区剩余大小
uint32_t CircularBuffer_GetRemainingSize() {
  if (m_cb.head >= m_cb.tail) {
    return BUFFER_SIZE - (m_cb.head - m_cb.tail);
  } else {
    return m_cb.tail - m_cb.head - 1;
  }
}
// 获取环形缓冲区已写入大小
uint32_t CircularBuffer_GetWrittenSize() {
  if (m_cb.head >= m_cb.tail) {
    return m_cb.head - m_cb.tail;
  } else {
    return BUFFER_SIZE - (m_cb.tail - m_cb.head - 1);
  }
}
// 向环形缓冲区写入单个数据
bool CircularBuffer_Write(uint8_t data) {
  if (CircularBuffer_IsFull()) {  // 缓冲区已满，无法写入
    return false;
  }
  m_cb.buffer[m_cb.head] = data;
  m_cb.head = ((m_cb.head) + 1) % BUFFER_SIZE;
  return true;
}

// 从环形缓冲区读取单个数据
bool CircularBuffer_Read(uint8_t* data) {
  if (CircularBuffer_IsEmpty()) {  // 缓冲区为空，无数据可读取
    return false;
  }

  *data = m_cb.buffer[m_cb.tail];
  m_cb.tail = (m_cb.tail + 1) % BUFFER_SIZE;
  return true;
}

// 从环形缓冲区读取指定长度的数据
bool CircularBuffer_ReadData(uint8_t* data, uint16_t length) {
  if (CircularBuffer_GetWrittenSize() < length) {
    return false;  // 缓冲区中的数据不足
  }

  for (uint16_t i = 0; i < length; ++i) {
    if (!CircularBuffer_Read(&data[i])) {
      return false;  // 读取数据出错
    }
  }
...
// 向环形缓冲区写入指定长度的数据
bool CircularBuffer_WriteData(uint8_t* data, uint16_t length) {
  if (CircularBuffer_GetRemainingSize() < length) {
    return false;  // 缓冲区剩余空间不足
  }

  for (uint16_t i = 0; i < length; ++i) {
    if (!CircularBuffer_Write(data[i])) {
      return false;  // 写入数据出错
    }
  }
...
uint8_t g_DeleteProOrder(uint8_t proNum, uint16_t list)
{
    ...
    if(m_OperateProOrder[list].cmd == 200)
    {//程序是结束指令，无法删除
        return 4;
    }
...
{//通过检测usb相应目录下是否有U盘插入
    DIR *DIR_PTR;
    int usb_bool =0;
    if((DIR_PTR = opendir("/sys/bus/usb/drivers/usb-storage/2-1:1.0")) == NULL)
    {
        usb_bool = 0;
    }
    else
...
**  开发日期：	2024.10.11
**************************************************************************/
int QProcess_execute(QString program,QStringList arguments)
{
    bool success = QProcess::execute(program, arguments);
    return success;
}
